...
public class FtqlUtility {
    ...
    public static HashMap<String, FluentTQLUserInterface> getSpecs(String path) {
        ...
        try {
            ...
            if (notLoadedFileList.size() != 0) {
                ...
                throw new Exception( "Below FluentTQL specifications are invalid.\n" + printList);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static List<TaintFlowQuery> getTaintFLowQuerySpecs(String tqlSpecPath) {
    	
    	HashMap<String, FluentTQLUserInterface> namedSpecs = FtqlUtility.getSpecs(tqlSpecPath);
    	
    	List<TaintFlowQuery> taintFlowSpecs = new ArrayList<>();
 	   
 	   // Get the TaintFLowQuery object.
        for (FluentTQLUserInterface fluentTQLInterface : namedSpecs.values()) {
     	   List<FluentTQLSpecification> fluentTQLSpecifications = fluentTQLInterface.getFluentTQLSpecification();
