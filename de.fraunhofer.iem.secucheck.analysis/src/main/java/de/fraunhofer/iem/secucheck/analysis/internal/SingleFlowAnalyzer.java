...
public class SingleFlowAnalyzer {
	...
	public SingleFlowAnalyzer(TaintFlowQuery singleFlow,
			ObservableICFG<Unit, SootMethod> icfg) {
		this.singleFlow = singleFlow;
		this.icfg = icfg;
	}
	
	public Map<TaintFlowQuery, Pair<Query, Query>> analyze() {
		
		Map<TaintFlowQuery, Pair<Query, Query>> allReachMap =
				new HashMap<TaintFlowQuery, Pair<Query, Query>>();
		
		for (TaintFlowQuery partialFlow : getLogicalSubFlows(singleFlow)) {
	...
	private Map<TaintFlowQuery, Pair<Query>> getReachingPairs(Boomerang boomerang, 
			TaintFlowQuery partialFlow, Set<? extends Query> queries,
			Set<? extends Query> reachable) {
		Map<TaintFlowQuery, Pair<Query>> reachMap = new HashMap<TaintFlowQuery, Pair<Query, Query>>();
		for (Query start : queries) {
			for (Query end : reachable) {
				if (isValidPath(boomerang, start, end)) {
					if (start instanceof ForwardQuery) {
						reachMap.put(partialFlow, new SameTypedPair<Query, Query>(start, end));
					} else if (start instanceof BackwardQuery) {
						reachMap.put(partialFlow, new SameTypedPair<Query, Query>(end, start));
					}
				}
			}
		}
